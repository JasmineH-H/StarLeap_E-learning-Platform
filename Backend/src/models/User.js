...
userSchema.pre("save", function (next) {
  if (
    ...
    this.assignedInstructors.length > 0
  ) {
    this.assignedInstructors = [];
  }
  next();
...
export const createUser = async (userData) => {
  ...
  if (!userName || !password || !role || !firstName || !lastName) {
    throw new Error(
      "Missing required fields: userName, password, role, firstName, lastName",
    );
  }
  ...
  const user = new User({
    ...
    assignedInstructors: Array.isArray(assignedInstructors)
      ? assignedInstructors
      : undefined,
  });
...
if (true) {
  (async () => {
    try {
      const MONGODB_URI =
        process.env.MONGODB_URI || "mongodb://localhost:27017/starleapauth";
      ...
      const instructorExisting = await User.findOne({
